...
Not queue-facing. Thrown during binary resolution at the point `BinaryResolver::resolve()` is called. Fix by ensuring the binary is installed (`composer require naoray/gaze-laravel` triggers the Composer plugin) or by setting `GAZE_BINARY` to a valid path.
